...
var Lifex_cost_GroupsController = (function() {
    var self = {};
    ...
    self.getCurrentGroupVarName = function() {
        ...
        if(Lifex_cost.groupByKey){key=Lifex_cost.groupByKey};
    ...
    self.setJstreeGroupTypes = function(groupKeys) {
        var allNodes = JstreeWidget.getNodeDescendants(Lifex_cost_FiltersWidget.jstreeDiv, "#");
        allNodes.forEach(function(node) {
            $("#" + Lifex_cost_FiltersWidget).jstree().set_type(node.id, "default");
        });
        var types = {};
        groupKeys.forEach(function(nodeId) {
            ...
            $("#" + Lifex_cost_FiltersWidget).jstree().set_type("default");
        });
    };
    ...
    self.getGroups = function(groupKey, callback) {
        ...
        if (!groupKey || groupKey == "Activity") {
            ...
            /*
            var groupNodes = JstreeWidget.getNodeDescendants(Lifex_cost_FiltersWidget.jstreeDiv, curentNodeid);
            ...
            var curentNodeid = Lifex_cost_FiltersWidget.currentNode.id;
            var groupNodes = JstreeWidget.getNodeDescendants(Lifex_cost_FiltersWidget.jstreeDiv, curentNodeid);
            groupNodes.forEach(function(node) {
                dataGroups[node.data.id] = { id: node.data.id, content: node.text, data: {} };
            ...
            var curentNodeid = Lifex_cost_FiltersWidget.currentNode.id;
            var groupNodes = JstreeWidget.getNodeDescendants(Lifex_cost_FiltersWidget.jstreeDiv, 'Resource');
            groupNodes.forEach(function(node) {
                dataGroups[node.data.id] = { id: node.data.id, content: node.text, data: {} };
            ...
            var groupNodes = JstreeWidget.getNodeDescendants(Lifex_cost_FiltersWidget.jstreeDiv, groupKey);
    ...
    self.getGroupFn = function(groupKey) {
        ...
        var belongsToGroupFn = function(groupValue, item) {
            var groupVarName = Lifex_cost_GroupsController.filterGroupsLabelsMap[groupKey];
            if (!groupKey || groupKey == "Activity") {
    ...
    self.legend = {
        getLegendClassesMap: function() {
            ...
            $(".vis-legend").find(".vis-legend-text").each(function() {
                var str = $(this).html();
...
export default Lifex_cost_GroupsController;
window.Lifex_cost_GroupsController = Lifex_cost_GroupsController;